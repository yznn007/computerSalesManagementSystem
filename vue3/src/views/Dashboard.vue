  ...
  ╚═════╝ ╚══════╝</pre>
      ...
      <div class="search-wrapper">
        <div class="search-box">
          ...
          <input v-model="keyword" class="search-input" placeholder="Search products..." @keydown.enter="search" />
        </div>
      </div>
    </div>
    ...
    <div class="product-section">
      ...
      <div v-loading="loading">
        <div v-for="p in displayedProducts" :key="p.product_id" class="row" @click="showDetail(p)">
          ...
          <button class="add-btn" :disabled="p.stock <= 0" @click.stop="addToCart(p)">+</button>
        </div>
      ...
      <el-dialog v-model="detailVisible" :title="detailProduct?.model" width="420px">
        ...
        <template #footer>
          <el-button v-if="detailProduct" @click="detailVisible = false" size="small">关闭</el-button>
          <el-button v-if="detailProduct" type="primary" size="small" :disabled="detailProduct.stock <= 0" @click="addToCart(detailProduct); detailVisible = false">加入购物车</el-button>
        </template>
      </el-dialog>
    </div>

    <button v-if="cartItems.length" class="cart-fab" @click="cartVisible = true">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
      <span class="cart-badge">{{ totalCount }}</span>
    </button>

    <el-drawer v-model="cartVisible" title="购物车" size="360px" direction="rtl">
      ...
      <div class="cart-body">
        ...
        <template v-else>
          ...
          <div class="cart-footer">
            ...
            <button class="submit" :disabled="(isStaff && !selectedCustomer) || submitting" @click="submitOrder">{{ submitting ? '...' : '提交订单' }}</button>
          </div>
        </template>
      </div>
    </el-drawer>
...
import { getProducts, getProductDetail, getCustomers, createOrder } from '../api'
...
const addToCart = (p) => {
  ...
  else { cartItems.value.push({ ...p, quantity: 1 }); ElMessage.success({ message: '已加入购物车', duration: 1000 }) }
}
...
const submitOrder = async () => {
  ...
  try {
    ...
    if (res.data?.status === 0) {
      ...
      cartItems.value = []; cartVisible.value = false; selectedCustomer.value = null; fetchProducts()
...
.ascii { font-family: var(--font-mono); font-size: 18px; line-height: 1.25; margin: 0 0 24px; letter-spacing: 0; background: linear-gradient(180deg, #d0d0d0 0%, #8a8a8a 40%, #5a5a5a 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
.hero-sub { font-size: 20px; font-weight: 500; color: #6a6a6a; letter-spacing: 2px; }
.search-wrapper { margin-top: 40px; display: flex; justify-content: center; }
.search-box { position: relative; width: 100%; max-width: 390px; }
.search-icon { position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: #4a4a4a; pointer-events: none; }
.search-input { width: 100%; padding: 12px 0 12px 28px; background: transparent; border: none; border-bottom: 1px solid #333; color: #d0d0d0; font-family: var(--font-mono); font-size: 14px; outline: none; caret-color: #d0d0d0; transition: border-color 0.2s; }
.search-input::placeholder { color: #4a4a4a; }
.search-input::selection { background: rgba(255, 255, 255, 0.2); }
...
.table-head { display: grid; grid-template-columns: 1fr 80px 60px 100px 40px; align-items: center; padding: 8px 8px; border-bottom: 1px solid #1a1a1a; }
...
.row { display: grid; grid-template-columns: 1fr 80px 60px 100px 40px; align-items: center; padding: 12px 8px; cursor: pointer; transition: background 0.1s; border-bottom: 1px solid #1a1a1a; }
...
.add-btn { width: 26px; height: 26px; border: none; background: none; color: var(--text-tertiary); cursor: pointer; border-radius: 3px; font-size: 16px; line-height: 1; display: flex; align-items: center; justify-content: center; transition: color 0.15s; }
.add-btn:hover:not(:disabled) { color: #e0e0e0; }
...
.cart-fab { position: fixed; right: 32px; bottom: 32px; width: 52px; height: 52px; border-radius: 50%; border: 1px solid #444; background: #1a1a1a; color: #e0e0e0; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 16px rgba(0,0,0,0.5); transition: all 0.15s; z-index: 100; }
.cart-fab:hover { background: #252525; border-color: #666; transform: scale(1.05); }
.cart-badge { position: absolute; top: -4px; right: -4px; min-width: 20px; height: 20px; padding: 0 5px; border-radius: 10px; background: #e0e0e0; color: #0a0a0a; font-size: 11px; font-family: var(--font-mono); font-weight: 600; display: flex; align-items: center; justify-content: center; }
.cart-block { margin-bottom: 20px; padding-bottom: 16px; border-bottom: 1px solid #333; }
.cart-label { font-size: 12px; color: var(--text-tertiary); margin-bottom: 8px; }
.cart-empty { text-align: center; color: var(--text-tertiary); padding: 60px 0; font-size: 13px; }
.cart-list { display: flex; flex-direction: column; gap: 4px; }
.cart-row { padding: 12px 0; border-bottom: 1px solid #2a2a2a; }
.cart-row:last-child { border-bottom: none; }
.cart-row-top { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 8px; }
...
.qty { width: 120px; }
.rm { background: none; border: none; font-size: 18px; color: var(--text-tertiary); cursor: pointer; margin-left: auto; }
.rm:hover { color: #f56c6c; }
.cart-footer { position: absolute; bottom: 0; left: 0; right: 0; padding: 16px 20px; border-top: 1px solid #333; background: #131313; }
...
.submit { width: 100%; padding: 10px 0; background: #e0e0e0; border: none; color: #0a0a0a; font-size: 14px; font-weight: 600; cursor: pointer; font-family: var(--font-sans); transition: opacity 0.15s; }
